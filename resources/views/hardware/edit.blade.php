...
@section('moar_scripts')
    ...
    <script nonce="{{ csrf_token() }}">
        ...
        function generateTag() {
            let modelId = $('#model_select_id').val()
            let companyId = $('#company_select').val()
            let isCreate = window.location.href.indexOf('/create') > -1
            let tag = $('#asset_tag')
            console.log(modelId, companyId,tag.val())
            if (!modelId || !companyId || !isCreate) {
                return 0;
            }
            $.ajax({
                type: 'GET',
                url: "{{ url('/') }}/hardware/generateTag",
        ...
        $(function () {
            ...
            $('#company_select').on('change', generateTag)
        ...
        $(document).ready(function() {
            ...
            $(add_button).click(function(e){ //on add input button click
                e.preventDefault();
                let match = /(.*?)(\d+$)/.exec($("#asset_tag").val());
                ...
                if (x < max_fields) {
                    let auto_tag = ''
                    if (match) {
                        auto_tag = match[1] + zeroPad(parseInt(match[2]) + parseInt(x), match[2].length);
                    } else {
                        auto_tag = '';
                    }
